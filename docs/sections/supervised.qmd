...
# Remove the "address" column from both datasets
...
properties_filtered_2 <- properties_filtered %>% filter(canton %in% c("Valais", "Ticino", "Vaud", "Bern", "Fribourg"))
#show each unique value in canton col
unique(properties_filtered_2$canton)
```
...
## Fit the Model
lm_model_1.1 <- lm(price ~ number_of_rooms + square_meters + property_type + floor + year_category  + Political_cluster + Tax_cluster + Demographic_cluster, data=trainData)
...
INTERPRETATION COMPARE BOTH MODELS

#### Assess Overfitting

```{r}
# For the Linear Model
lm_train_pred <- predict(lm_model_1.1, newdata = trainData)
lm_test_pred <- predict(lm_model_1.1, newdata = testData)
...
rmse <- rmse(testData$price, predict(lm_model_1.1))
mae <- mae(testData$price, predict(lm_model_1.1, newdata=testData))
...
rmse <- rmse(testData$price, predict(lm_model_2.1))
mae <- mae(testData$price, predict(lm_model_2.1, newdata=testData))
...
Here was the previous metrics of our stepwise model (without selecting for cantons)
```{r}
metrics_2
```
...
```{r}
## Cross-Validation
cv_results2 <- train(price ~ number_of_rooms + square_meters + year_category + property_type + Demographic_cluster +Political_cluster + Tax_cluster, data=trainData, method="lm", trControl=trainControl(method="cv", number=10))
summary(cv_results2)
...
metrics_cv2 <- kable(cv_results2$results, format = "html") %>%
  ...
  kable_styling(position = "center", bootstrap_options = c("striped", "bordered", "hover", "condensed"))  %>%
  add_header_above(c("10 Fold Cross Validation Metrics (Selected Canton)" = 7))
metrics_cv2
```
...
# Model Testing 
## Test the Model
predicted_prices <- predict(lm_model_2.1, newdata=testData)
testData$predicted_prices <- predicted_prices  # Add to testData to ensure alignment
# Calculate residuals
testData$test_residuals <- testData$price - predicted_prices  # Manually compute residuals
...
# Remove the "address" column from both datasets
trainData <- trainData[, !(names(trainData) %in% c("address"))]
testData <- testData[, !(names(testData) %in% c("address"))]
...
rf_tuned <- train(price ~ number_of_rooms + square_meters + canton + floor + year_category + Demographic_cluster + Political_cluster+ Tax_cluster, data = trainData, method = "rf", 
