...
The Stewise model is thus the best model because it has the lowest RMSE and MAE values on the test and training set.
**CHANGE FOR WHEN WE HAVE CORRECT DATAwe observe that the Lasso model outperforms the Ridge model in terms of RMSE and R-squared values, indicating that Lasso regularization may be more effective in this context.**
