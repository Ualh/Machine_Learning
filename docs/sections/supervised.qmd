...
```{r}
#filter price on less than 3.3M
properties_filtered_3 <- properties_filtered_2 %>% filter(price < 3300000)
# Model Building
...
#apply the RF model as a regression
rf_model <- randomForest(price ~ square_meters + property_type +  year_category  + Political_cluster + Tax_cluster + Demographic_cluster, data=testData3, ntree=500, importance=TRUE)
...
```{r}
## Cross-Validation
cv_results2 <- train(price ~ square_meters + year_category + property_type + Demographic_cluster +Political_cluster + Tax_cluster, data=trainData3, method="lm", trControl=trainControl(method="cv", number=10))
summary(cv_results2)
