...
```
We choose to only display the properties type with more than 300 observations, but you can find other type such as rustic house, terrace flat, loft or farm house.
...
```
We choose to only show 1 category because the majority of properties were built between 2016 and 2024. And the distribution remains similar across almost all periods so it make sens to only show the relevant graph
...
```
Note that we didn't show the properties with more than 12 rooms because we have less observations and we understand the tendency with those graphs.
