...
Next, we will interpret the clusters by looking at the cluster centers, the size of each cluster, and the distribution of the variables within each cluster.
```{r}
...
And then we will plot the boxplots of the variables in each cluster to interpret the clusters.
```{r}
...
The same process was used for our political data set, with 5 clusters for the same reasons. The share of each major parties voted for the Conseil National are represented. The only difference was that we did not scale our data, as all features are percentages.
```{r}
# Clustering politics
